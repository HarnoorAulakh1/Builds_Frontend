...
function Heading() {
  ...
  async function addFriend() {
    ...
    const response = await axios.post(`/api/user/addFriendRequest`, {
      user_id: _id,
      profile_link: user._id,
      type: "friend_request",
      content: `Friend request from ${user.username}`,
      photo: user.photo,
      profile_username: user.username,
    });
    if (response.data == "Already Friends") alert("Already Friends");
  ...
  return (
    <div className={styles.container}>
      <div className={styles.header}>
        ...
        <div>
          {_id == user._id && (
            <button onClick={() => handlelogout()} className={styles.addpost}>
              Logout
            </button>
          )}
        </div>
